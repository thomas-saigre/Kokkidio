...
_Kokkidio_ provides 
an iteration range class and parallel dispatch functions compatible with it.
...
// See its <<_parrange, dedicated section>> for more details.
...
Firstly, it contains the index or index range associated with a thread,
...
returns either a single element, if that object is one-dimensional,
or a column expression, if that object is two-dimensional.
By default, 
...
On a GPU, this is 
a single element of a 1D array/vector, or
a single column of a 2D array/matrix.
On a CPU
...
(first) argument. 
On the host (CPU), this leads to a different execution path,
as here, parallel execution with a `ParallelRange` functor always uses OpenMP.
If a different functor is provided, they forward to their Kokkos equivalents.


On host, parallel execution generally uses OpenMP, 
because of its wide availability and compatibility.

Kokkos' functions, if the functor passed to them takes a
`ParallelRange` as its argument. Otherwise, it forwards to Kokkos' functions.
