...
and thus sweeping changes to the whole library would be necessary.
...
It takes the same template parameters as <<_mapview,`ViewMap`>>, 
...
Similar to <<_mapview,`ViewMap`>>, it also allows to `resize()` its data,
...
The `ViewMap` and `DualViewMap` classes can be instantiated in a similar fashion to `Eigen` classes, 
with similar behaviour:

```c++
using namespace Kokkidio;
ViewMap<Eigen::MatrixXd>
	a, // default construction, no allocation
	b (2, 3); // constructs 
```
and [`DualViewMap`] are designed to function  
Here's an example using a `DualViewMap`:
...
/* The DualViewMap constructors which take sizes allocate memory 
 * on both host and target: */
MatrixView mat (nRows, nCols);

/* The DualViewMap constructors which take Eigen objects only allocate
